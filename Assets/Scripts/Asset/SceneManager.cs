...
public class SceneManager
{
    ...
    /// 一个场景里面所有的资源包
    /// </summary>
    private OneSceneAB sceneAssetBundles;
    ...
    {
        sceneAssetBundles = new OneSceneAB(sceneName);
        ...
        using (FileStream fs = new FileStream(path,FileMode.Open,FileAccess.Read))
        {
            using (StreamReader sr = new StreamReader(fs))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    string[] kv = line.Split(new string[] { "--" },System.StringSplitOptions.RemoveEmptyEntries);
